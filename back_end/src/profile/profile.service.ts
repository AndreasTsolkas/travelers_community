...
export class ProfileService {
  ...
  async findOneWithRelationshipsBySpecificFieldAndValue(
    field: string,
    value: any,
  ): Promise<User | null> {
    try {
      return await this.userService.findOneWithRelationshipsBySpecificFieldAndValue(
        field,
        value,
        true,
      );
  ...
  async checkIfPasswordIsCorrect(id: number, password: string) {
    if (!(await this.userService.checkIfPasswordIsCorrect(id, password)))
      throw new BadRequestException(
        'The password you sent is not the correct.',
      );
  }
  ...
  async getAvatarRelativeFilePath(userId: number): Promise<string | undefined> {
    ...
    if (user) result = 'app_images/' + user?.avatarFilepath + '.jpg';
    return result;
  }
  ...
  async getAvatarFilePath(userId: any): Promise<string> {
    ...
    const absolutePath: string =
      await this.getAvatarAbsolutePath(relativeFilePath);
    return absolutePath;
  }
